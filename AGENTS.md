...
>
> 现有 API：`GET/POST /api/groups`（`GET` 返回每组的 `count` 与 `covers` 封面图 URL，聚合取每类最新 4 张，`$topN` 实现）、`PATCH/DELETE /api/groups/:id`、`GET /api/memes`（按 `group` 过滤 + `limit`/`offset` 分页，最近使用分组附加 50 上限；支持 `tags` 参数逗号分隔做**交集筛选**、`q` 参数按名称/标签大小写不敏感子串搜索）、`POST /api/memes`（multipart 多文件上传，`groupId` 可选，缺省入未分组；逐文件校验**格式 / 空文件 / ≤20MB / 最长边 ≤2560px**（`storage::MAX_IMAGE_EDGE`，经 `probe_dimensions` 仅读图片头不全量解码），返回 `{ results: [{ name, status: "created"|"failed", reason }] }`，单个坏文件不阻断整批）、`PATCH/DELETE /api/memes/:id`（改名/移动/收藏切换/整体替换 `tags`（去重去空、单标签 ≤30 字符、上限 20 个），删除时清理原图+缩略图）、`POST /api/memes/:id/use`（记录最近使用）、`GET /api/memes/:id/file`（原图，支持 `Range`，`cache-control: private, immutable`）、`GET /api/memes/:id/thumb`（缩略图，无缩略图时回退原图）、`GET /api/tags`（聚合全部标签及使用次数 `[{ name, count }]`，按次数降序）、`GET /api/overview`（表情总数/收藏数/分组数（不含全部系统分组）/存储占用，5s TTL 缓存）、`GET /api/health`（公开健康检查，心跳检测用）。
> 前端已接入 API：`useMemes()` 拉取分组并提供分组/表情的增删改，分组详情页为**无限滚动**（`useIntersectionObserver` + 底部哨兵，滚动自动加载下一批），内置**搜索框（300ms 防抖）与标签筛选条**（`GET /api/tags` 聚合驱动，可多选交集，编辑标签经 `TagEditorModal` 整体替换后 revision 联动刷新标签条）；表情卡片用原生 `<img>` 直连文件端点（避免 IPX 二次代理），**GIF 动画预览默认开启并用原图 URL 保留动画；设置关闭时改用首帧缩略图，点击复制仍始终读取原始 GIF**，其他格式走缩略图，卡片底部展示前 3 个标签（多余折叠为 `+N`）。**导入体验（桌面端）**：文件收集统一走 `tauri-app/src/composables/useFileCollect.ts`（`collectFromDataTransfer` 递归展开拖入的文件夹、`collectFromInput` 按 `webkitRelativePath` 还原所属文件夹、`screenFiles` 做格式/空/体积/**分辨率**预校验并逐项给出拒绝原因）；上传弹窗提供「选择文件 / 选择文件夹」双入口，`DropImportOverlay.vue` 挂在 `layouts/default.vue` 提供**窗口级拖放导入**（`tauri.conf.json` 必须保持 `dragDropEnabled: false`，否则 webview 收不到 HTML5 拖放事件；弹窗打开时窗口级拖放让位给弹窗内拖放区，避免重复入列），落地分组为当前分组页（收藏/最近使用除外）否则未分组；文件夹导入可勾选**按文件夹名自动建组**（`useMemes().ensureGroup` 同名复用、409 以服务端为准），`buildBatches` 保证同一批次不混用目标分组。上传逻辑收敛在双端 `useUpload.ts`（XHR 字节级进度、客户端预校验、失败逐项展示原因 + 一键重试、按 20 个/100MB 分批限速、单次上限 500 个）。ObjectId 校验统一走 `nuxt-app/server/utils/validate.ts` 的 `isValidId`/`requireValidId`；前端错误解析统一走 `app/utils/error.ts` 的 `getErrorMessage` + `useAsyncAction`。
